...
function [wk, stat] = MM_gl_l1(X_noisy, a, b,w_0,thresh,nSamples)
...
while eta>thresh
    ...
    Swki=1./Swk;
    Swkii=repmat(Swki,[1 m]);
    obj_val(idx) = (2*u'*d)+(b*(norm(u,1)))-(a*sum(log(S*u))); % + c*sum(S*u); % MODIFIED HERE FROM L2 to L1
    tim(idx)=toc;

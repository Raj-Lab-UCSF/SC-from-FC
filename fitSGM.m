...
%               params.TR = The signals TR
%               params.fband = Two element vector where the first element
...
%                   field isparams provided, then no fitting will occur!
%           2. A scalar representing the TR of the time series. In this
%               case, default settings will be used.
% 
%   Ouputs:
...
% Model initial parameters - change if necessary
    theta0 = [0.5, 1];
    ll = [0.1, 0.1];
    ul = [10, 5];
...
% % % % Verify that the number of regions in TS and SC are the same:
    ...
    if ~isequal(sz_TS(2), nroi) && ~isequal(sz_TS(1), nroi)
        error('Number of regions in the timeseries does not equal the number of regions in the structure!');
...
% % % % Check the params variable:
    if isstruct(params)
        ...
        if ~isfield(params,'TR')
            error("Signal's sampling rate (TR), not provided");
        end
...
% % % % SGM Setup:
    ...
    if (nt < 64) && (strcmp(params.model_focus, 'both') || strcmp(params.model_focus, 'FX'))
        warning('Not enough timepoints for a good FFT; SGM is only fitting to FC.')
...
if ~forward_only % was ~forward_only
    disp('this is forward mode')
